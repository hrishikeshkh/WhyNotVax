...
df_pharma = data.copy()
df_pharma["pharma"] = df_pharma["labels"].apply(
    lambda x: 1 if "pharma" in x else 0
)
...
df_country["country"] = df_country["labels"].apply(
...
df_rushed = data.copy()
df_rushed["rushed"] = df_rushed["labels"].apply(
    lambda x: 1 if "rushed" in x else 0
)
...
df_none = data.copy()
df_none["none"] = df_none["labels"].apply(
    lambda x: 1 if "none" in x else 0
)
